...
async fn handshake_emits_challenge_credential_and_receipt_events() {
    ...
    timeout(TIMEOUT, handle.receipt.changed())
        .await
        .unwrap()
        .unwrap();
    ...
    let e1 = timeout(TIMEOUT, handle.events.recv())
        .await
        .unwrap()
        .unwrap();
    assert!(matches!(e1, MppEvent::Challenge(_)));
    let e2 = timeout(TIMEOUT, handle.events.recv())
        .await
        .unwrap()
        .unwrap();
    assert!(matches!(e2, MppEvent::CredentialSent));
    let e3 = timeout(TIMEOUT, handle.events.recv())
        .await
        .unwrap()
        .unwrap();
...
async fn json_rpc_round_trips_through_mpp_message_envelope() {
    ...
    let frontend = MppWsConnect::new(url, StubProvider)
        .into_service()
        .await
        .unwrap();
...
async fn need_voucher_without_provider_surfaces_error_event() {
    ...
    assert!(
        saw_error,
        "expected Error event when NoVoucher provider rejects"
    );
}
...
async fn duplicate_challenge_or_voucher_closes_connection() {
    ...
    impl PaymentProvider for BlockingProvider {
        ...
        async fn pay(&self, ch: &PaymentChallenge) -> Result<PaymentCredential, MppError> {
            ...
            Ok(PaymentCredential::new(
                ch.to_echo(),
                PaymentPayload::hash("0xfeedface"),
            ))
        }
    }
...
async fn duplicate_need_voucher_closes_connection() {
    ...
    assert!(
        closed,
        "translator should close after duplicate NeedVoucher"
    );
}
...
async fn payment_provider_error_surfaces_event_and_closes() {
    ...
    assert!(
        saw_error,
        "expected Error event when payment provider returns Err"
    );
}
...
async fn voucher_provider_error_surfaces_event_and_closes() {
    ...
    assert!(
        saw_error,
        "expected Error event when voucher provider returns Err"
    );
}
...
async fn multiple_json_rpc_requests_round_trip_in_session() {
    ...
    let frontend = MppWsConnect::new(url, StubProvider)
        ...
        .unwrap();
    for i in 1..=3u64 {
        let req = Request::new("eth_blockNumber", Id::Number(i), ())
            .serialize()
            .unwrap();
        let resp = timeout(TIMEOUT, frontend.send(req)).await.unwrap().unwrap();
        let payload = resp.payload.as_success().unwrap();
...
async fn server_error_frame_surfaces_event_and_closes() {
    ...
    let ev = timeout(TIMEOUT, handle.events.recv())
        .await
        .unwrap()
        .unwrap();
    match ev {
...
async fn fatal_provider_error_does_not_re_invoke_pay() {
    ...
    assert_eq!(
        ...
        1,
        "fatal error must not trigger reconnects"
    );
    assert_eq!(
        pay_calls.load(Ordering::SeqCst),
        1,
        "pay() must not be re-invoked"
    );
}
